...
fn main() {
    ...
    if let Ok(i) = args.binary_search(&"fn".to_string()) {
        if args.len() < i + 2 {
            panic!("Not enough argument provided");
        }
        match args[i + 1].as_str() {
            "user_stream" => user_stream(),
            "user_stream_websocket" => user_stream_websocket(),
            "market_websocket" => market_websocket(),
            "all_trades_websocket" => all_trades_websocket(),
            "kline_websocket" => kline_websocket(),
            "last_price_for_one_symbol" => last_price_for_one_symbol(),
            "multiple_streams" => multiple_streams(),
            _ => panic!("Unknown function asked: {}", args[i + 1]),
        }
    } else {
        kline_websocket()
    };
